...
**ALL PROJECTS MUST SUPPORT BOTH DARK AND LIGHT MODES**

- Never create single-mode projects (light-only or dark-only)
- Use CSS custom properties (HSL format) for all theme colors
- Implement theme toggle component in the UI
- Support three modes: light, dark, and system (follows OS preference)
- Persist user's theme choice in localStorage
- Ensure smooth transitions between themes (200-300ms)
- Test all UI components in both themes during development
...
- [ ] CSS custom properties for light mode defined in `:root`
- [ ] CSS custom properties for dark mode defined in `.dark`
